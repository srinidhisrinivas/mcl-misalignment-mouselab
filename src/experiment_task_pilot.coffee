...
initializeExperiment = ->
  ...
  task_combined["experiment_instructions"] = {
    type: jsPsychInstructions
    on_start: () ->
      psiturk.finishInstructions() #started instructions, so no longer worth keeping in database
    show_clickable_nav: true
    data:
      trial_id: "exp_instructions_combined"
    pages: -> [
      """
        <h1> Instructions </h1>
  ...
  task_disappearing_timed["mouselab_instructions_1"] = {
    ...
    pages: -> [
      ...
      """
        ...
        of the arrows between the nodes. The image below shows the shape of all the webs that you will be navigating in when the game starts. For the sake of illustration, each node in this image is labeled by the number of steps it is away from the starting node (1 to 5).

       <img class='display' style="width:50%; height:auto" src='static/images/web-of-cash-5-levels-2.png'/>
      ...
      """
        ...
        <br><br>
        The node inspector costs a certain amount to reveal a node. This cost may vary between nodes. The fee will be instantly deducted from the spider's money (your score) for that round.
        <br><br>
  ...
  task_control["mouselab_instructions_1"] = {
    ...
    pages: -> [
      ...
      """
        ...
        To help you understand the game, it would be helpful to have some practice rounds. The following two rounds will give you a chance to practice playing the game.
        <br> <br>
        However, the practice rounds will differ from the actual rounds of the game in one important respect: <br><br>
        <ol>
          <li>the values at the nodes have the same magnitude (either $10 or $-10). This will <strong>NOT</strong> be the case in the actual rounds, and <strong>the values of the nodes in the actual game will instead vary between the nodes.</strong></li>
          <li>the cost to use the node inspector is fixed at $0.50. This will <strong>NOT</strong> be the case in the actual rounds, and <strong>the cost of the nodes in the actual game may vary between the nodes.</strong></li>
        </ol>
  ...
  task_combined["mouselab_instructions_1"] = {
    type: jsPsychInstructions
    data:
      trial_id: "mouselab_instructions_1_combined"
    show_clickable_nav: true
    pages: -> [
      ...
      """
        <h1> Web of Cash</em> (2/4) - Rewards and Costs </h1>
  ...
  task_control["practice_trials"] = {
    ...
    showCost: true
    lowerMessage: """
      Click on the nodes to reveal their values for a fee of $1.<br>
      Move with the arrow keys after you are done clicking.
        """
    scoreShift: 2
    stateBorder : () -> "rgb(187,187,187,1)"#getColor
  ...
  # Second set of mouselab instructions for the control condition
  ...
  # Second set of mouselab instructions for the combined condition
  task_combined["mouselab_instructions_2"] = {
    type: jsPsychInstructions
    data:
      trial_id: "mouselab_instructions_2_combined"
    show_clickable_nav: true
    pages: -> [
      """
        <h1> <em>Web of Cash</em> (3/4) - Memory Task </h1>
  ...
  # Mouselab instructions quiz for memory condition
  task_memory["mouselab_quiz"] = {
    ...
    data: {
      correct: {
        ...
        Q3: "To get the highest total score."
        Q4: 'Only before moving the spider.'
        Q5: "Simultaneously with each round of the Web of Cash game"
      }
      trial_id: "mouselab_quiz_memory"
    }
  }
  # Mouselab instructions quiz for control condition
  task_control["mouselab_quiz"] = {
    ...
    data: {
      correct: {
        ...
        Q1: 'The better I perform the higher my bonus will be.'
        Q2: 'No, the amount of cash at each node of the web may be different each time.'
        Q3: 'Only before moving the spider.'
        Q4: "To get the highest total score."
      }
      trial_id: "mouselab_quiz_control"
    }
  }
  # Mouselab instructions quiz for combined condition
  task_combined["mouselab_quiz"] = {
    preamble: ->  """
      <h1> Quiz </h1>
    ...
    data: {
      ...
      trial_id: "mouselab_quiz_combined"
    }
  }
  ...
  task_combined["mouselab_instruct_loop"] = {
    timeline: [
      fullscreen,
      task_combined["mouselab_instructions_1"],
      task_combined["practice_trials"],
  ...
  task_disappearing_timed["final_quiz"] =
    ...
    questions: [
      {prompt: "What is the range of node values in the first step (closest to the start, in the center)?", options: ['$-16 to 16', '$-2 to $2', '$-64 to $64'], required: true}
      {prompt: "What is the range of node values in the fourth step?", options: ['$-4 to $4', '$-16 to $16', '$-32 to $32'], required: true}
      {prompt: "What is the range of node values in the last step (furthest from the start, the edges)?", options: ['$-32 to $32', '$-16 to $16', '$-64 to $64'], required: true}
  ...
  task_control["final_quiz"] =
    ...
    preamble: -> """
      ...
      Your total score for the <em>Web of Cash</em> game was <strong>$#{SCORE}</strong>. The bonus that you receive will be based on this score.
      <br><br>
  ...
  task_combined["if_node2"] =
    timeline: [ready_screen, task_combined["test_trials"], task_combined["final_quiz"], self_report, demographics, finish]
    conditional_function: ->
      if REPETITIONS > MAX_REPETITIONS
        return false
      else
