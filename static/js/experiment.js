...
initializeExperiment = function() {
  var combinedTimelineVariables, demographics, experiment_timeline, finish, finish_fail, fullscreen, i, if_node1, k, mdp_trials, minimumTime, numSeqTrials, prompt_resubmit, ready_screen, ref, reprompt, save_data, secret_code_trial, self_report, stimulus_template, task_combined, task_control, task_disappearing_timed, task_memory, tv;
  $('#jspsych-target').html('');
  //  ============================== #
  //  ========= EXPERIMENT ========= #
  //  ============================== #
...
Move with the arrow keys after you are done clicking.`,
  ...
  task_control["practice_trials"] = {
    ...
    withholdReward: false,
    showCost: true,
    lowerMessage: `Click on the nodes to reveal their values for a fee of $1.<br>
Move with the arrow keys after you are done clicking.`,
    scoreShift: 2,
    stateBorder: function() {
      return "rgb(187,187,187,1)"; //getColor
...
`;
    ...
    questions: [
      ...
      {
        prompt: "When does each round of the memory task take place?",
        options: ["Before each round of the Web of Cash game",
      "Simultaneously with each round of the Web of Cash game",
      "After the Web of Cash game."],
        horizontal: false,
        required: true
      }
    ],
...
`;
  ...
  task_disappearing_timed["mouselab_instruct_loop"] = {
    timeline: [fullscreen, task_disappearing_timed["mouselab_instructions_1"], task_disappearing_timed["practice_trials"], task_disappearing_timed["mouselab_instructions_2"], task_disappearing_timed["mouselab_quiz"]],
    conditional_function: function() {
  ...
  task_memory["mouselab_instruct_loop"] = {
    timeline: [fullscreen, task_memory["mouselab_instructions_1"], task_memory["practice_trials"], task_memory["mouselab_instructions_2"], task_memory["mouselab_quiz"]],
    conditional_function: function() {
      if (DEBUG_INSTRUCTIONS) {
        return true;
      } else {
  ...
  task_control["mouselab_instruct_loop"] = {
    timeline: [fullscreen, task_control["mouselab_instructions_1"], task_control["practice_trials"], task_control["mouselab_instructions_2"], task_control["mouselab_quiz"]],
    conditional_function: function() {
      if (DEBUG_INSTRUCTIONS) {
        return true;
      } else {
  ...
  task_combined["mouselab_instruct_loop"] = {
    timeline: [fullscreen, task_combined["mouselab_instructions_1"], task_combined["practice_trials"], task_combined["mouselab_instructions_2"], task_combined["mouselab_quiz"]],
    conditional_function: function() {
      if (DEBUG_INSTRUCTIONS) {
        return true;
      } else {
...
(If, at any point, the <code>space</code> key does not take you to the next page, click once on the text and try again.)`
  ...
  for (i = k = 0, ref = NUM_TRIALS; (0 <= ref ? k < ref : k > ref); i = 0 <= ref ? ++k : --k) {
    tv = {
    ...
    timeline: [
      {
        ...
        stimulus: function() {
          return stimulus_template.replace("{sequence}",
...
Move with the arrow keys after you are done clicking.`,
        stateRewards: function() {
          return jsPsych.timelineVariable("stateRewards",
      true);
        },
        trial_id: function() {
...
<p>
  ...
  if (CONDITION === 0) {
    experiment_timeline = [task_control["experiment_instructions"], task_control["mouselab_instruct_loop"], if_node1, task_control["if_node2"]];
  } else if (CONDITION === 1) {
    experiment_timeline = [task_disappearing_timed["experiment_instructions"], task_disappearing_timed["mouselab_instruct_loop"], if_node1, task_disappearing_timed["if_node2"]];
  } else if (CONDITION === -1) {
    experiment_timeline = [task_memory["experiment_instructions"], task_memory["mouselab_instruct_loop"], if_node1, task_memory["if_node2"]];
  } else if (CONDITION === 2) {
    experiment_timeline = [task_combined["experiment_instructions"], task_combined["mouselab_instruct_loop"], if_node1, task_combined["if_node2"]];
  }
  // ================================================ #
